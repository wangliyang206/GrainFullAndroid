...
public class CombatAircraft extends Sprite {
    ...
    @Override
    protected void beforeDraw(Canvas canvas, Paint paint, AircraftWar gameView) {
        if (!isDestroyed()) {
            //确保战斗机完全位于Canvas范围内
    ...
    //发射子弹
    public void fight(AircraftWar gameView) {
    ...
    //战斗机爆炸
    private void explode(AircraftWar gameView) {
        if (!collide) {
            collide = true;
            setVisibility(false);
    ...
    //战斗机使用炸弹
    public void bomb(AircraftWar gameView) {
        if (collide || isDestroyed()) {
            return;
        }
