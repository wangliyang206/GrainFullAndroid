...
import static com.zqw.mobile.grainfull.mvp.ui.widget.particle.NumberView.DOWN_ANIMATOR_MODE;
import static com.zqw.mobile.grainfull.mvp.ui.widget.particle.NumberView.UP_ANIMATOR_MODE;
...
public class AnimNumberView extends LinearLayout {
    ...
    private int numberWidth = DensityUtils.dip2px(getContext(), 46);
    private int numberHeight = DensityUtils.dip2px(getContext(), 95);
    ...
    public void setTemperature(int temperature, int mode) {
        ...
        checkMode(mode);
        //设置控件的尺寸
        numberWidth = DensityUtils.dip2px(getContext(), 46);
        numberHeight = DensityUtils.dip2px(getContext(), 95);
        numberTextSize = 80;
        setLayoutSize(numberWidth, numberHeight, numberTextSize);
    ...
    public void setTimer(int second, int timerMode) {
        ...
        mTimerDisposable = Flowable.intervalRange(mCurrSecond,
                        second + 1,
                        0,
                        1,
                        TimeUnit.SECONDS)
    ...
    private void setClock(int second, int mode) {
        ...
        if (mHour1Value > 0) {
            ...
            //展示五位数字
            numberWidth = DensityUtils.dip2px(getContext(), 41);
            numberHeight = DensityUtils.dip2px(getContext(), 85);
            numberTextSize = 72;
            ...
            //展示三位数字
            numberWidth = DensityUtils.dip2px(getContext(), 56);
            numberHeight = DensityUtils.dip2px(getContext(), 114);
            numberTextSize = 92;
            mHour1View.setVisibility(GONE);
            mHour2View.setVisibility(GONE);
            mColon1View.setVisibility(GONE);
     ...
     * 设置当前显示的时间(秒)
     */
    public void setmCurrSecond(int mCurrSecond) {
        this.mCurrSecond = mCurrSecond;
        setClock(mCurrSecond, UP_ANIMATOR_MODE);
    }
}
