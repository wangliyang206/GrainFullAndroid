...
import com.cjt2325.cameralibrary.util.EToast2;
...
//                destroyCameraInterface();
    ...
    public void takePicture(final TakePictureCallback callback) {
        if (mCamera == null) {
            Toast.makeText(mContext, "请开起拍照权限!", EToast2.LENGTH_SHORT).show();
            return;
        }
        switch (cameraAngle) {
            case 90:
...
//
        ...
        mCamera.takePicture(null, null, new Camera.PictureCallback() {
            ...
            public void onPictureTaken(byte[] data, Camera camera) {
                ...
                //保存照片
                String jpegName = FileUtil.saveBitmap(saveVideoPath, bitmap);

                if (callback != null) {
                    if (nowAngle == 90 || nowAngle == 270) {
                        callback.captureResult(bitmap, true, jpegName);
                    } else {
                        callback.captureResult(bitmap, false, jpegName);
                    }
                }
            }
        });
...
//        }
        ...
        if (DeviceUtil.isHuaWeiRongyao()) {
            mediaRecorder.setVideoEncodingBitRate(JCameraView.MEDIA_QUALITY_FUNNY);
        } else {
    ...
    public interface TakePictureCallback {
        void captureResult(Bitmap bitmap, boolean isVertical, String path);
    }
